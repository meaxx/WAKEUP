...
void Loadimage()
{
    ...
    //景五
    loadimage(&POSTER, "image\\scene5\\海报.png");
    loadimage(&NINE, "image\\scene5\\九宫格.png");
    loadimage(&DIE, "image\\scene5\\死亡证明书.png");
    loadimage(&SAFEBOX, "image\\scene5\\密码箱.png");
    loadimage(&SAFEBOXIN, "image\\scene5\\密码箱开箱.png");
    loadimage(&SAFEBOXUP, "image\\scene5\\密码箱正面.png");
    loadimage(&PROP[5][1] ,"image\\scene5\\钥匙.png");
...
void Scene1()
{
    ...
    while (true)
    {
        ...
        {
            propbool[1][1] = 0;
            loadimage(&FLOWER, "image\\scene1\\苹果树.png");
            putimage(770, 300, &FLOWER);
            Bag(1);
            Bag(1);
        }
        if (seeduse && lineopen && open == 4&& m.x >= 1012 && m.x <= 1040 && m.y >= 408 && m.y <= 425)
        {
            propbool[1][6] = 1;
            loadimage(&FLOWER, "image\\scene1\\苹果树少苹果.png");
            putimage(770, 300, &FLOWER);
            Bag(1);
            Bag(1);
        }
...
void Scene5()//户外
{
    ...
    while (true)
    {
        ...
        if (m.uMsg == WM_LBUTTONDOWN)
        {
            ...
            if (m.x >= 469 && m.x <= 517 && m.y >= 204 && m.y <= 269)//海报  1
            {
                if (open != 1)
                {
                    putimage(770, 300, &POSTER);
                    open = 1;
                }
                else
                {
            ...
            if (m.x >= 245 && m.x <= 487 && m.y >= 458 && m.y <= 700)//九宫格 2
            {
                if (open != 2)
                {
                    putimage(770, 300, &NINE);
                    open = 2;
                }
                else
                {
            ...
            if (open == 2 && m.x >= 920 && m.x <= 1160 && m.y >= 328 && m.y <= 568)
            {
                int t = ((m.x - 920) / 80) + 1 + 3 * ((m.y - 328) / 80);
                password=password*10+t;
                s++;
                if (s == 6)
                {
                    s = 0;
            ...
            if (open == 2 && m.x >= 992 && m.x <= 1090 && m.y >= 583 && m.y <= 626)
            {
                s = 0;
                password = 0;
            }
            if (m.x >= 569 && m.x <= 604 && m.y >= 334 && m.y <= 360)//保险箱 4
            {
                if (open != 4)
                {
                    putimage(770, 300, &SAFEBOX);
                    open = 4;
                }
                else
                {
            ...
            if (open==4&&m.x >= 966 && m.x <= 1056 && m.y >= 424 && m.y <= 550)//保险箱 5
            {
                putimage(770, 300, &SAFEBOXUP);
                open = 4.1;
            }
            if (open == 4.1 && m.x >= 966 && m.x <= 1056 && m.y >= 424 && m.y <= 550)//保险箱 5
            {
                if (!opensafebox)
                {
                    putimage(770, 300, &SAFEBOXUP);
            ...
            if (open == 4.1&&opensafebox && m.x >= 1010 && m.x <= 1100 && m.y >= 488 && m.y <= 508)//拿钥匙
            {
                loadimage(&SAFEBOXIN, "image\\scene5\\密码箱开箱11.png");
                putimage(770, 300, &SAFEBOXIN);
                propbool[5][1] = 1;
                Bag(5);
                Bag(5);
            }
            if (m.x >= 293 && m.x <= 436 && m.y >= 172 && m.y <= 304&&propbool[5][1])
            {
                Scene6();
            }
        }
...
void Select(MOUSEMSG m)
{
    if (m.y >= 768 && m.y <= 790)
    {
        ...
        if (m.x >= 1032 && m.x <= 1080)
        {
            Scene2();
        }
        if (m.x >= 1095 && m.x <= 1144)
        {
            Scene3();
        }
        if (m.x >= 1157 && m.x <= 1210)
        {
            Scene4();
        }
        if (m.x >= 1220 && m.x <= 1270)
        {
            Scene5();
        }
        if (m.x >=1284 && m.x <= 1333)
        {
            Scene6();
        }
    }
